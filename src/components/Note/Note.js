...
const Note = ({ note, index, setEditNote, deleteNote, setDone }) => {
  ...
  return (
    <View
      key={index}
      style={[!note.important ? styles.note : styles.note__important]}
    >
      <Text
        style={[
          !note.important
            ? styles.note__content
            : styles.note__content__important,
          note.done && styles.note__content__done,
        ]}
      ...
      <View style={styles.note__crudButtons}>
        ...
        >
          <Text
            style={[
              !note.important
                ? styles.note__crudButtons__button
                : styles.note__crudButtons__button__important,
            ]}
          >
        ...
        <TouchableOpacity
          onPress={() =>
            Alert.alert(
              "Borrar Nota",
              "¿Estás seguro que querés borrar esta nota?",
              [
                {
                  text: "Cancelar",
                  style: "cancel",
